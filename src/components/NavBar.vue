    ...
    <div class="container-fluid max-w1550">
      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          ...
          <li class="nav-item">
            <router-link class="nav-link text-light" to="/users" v-if="role === Roles.ADMIN"
              >Χρήστες<i class="fa-solid fa-users ms-1"></i
            ></router-link>
          </li>
          <li class="nav-item">
            <router-link class="nav-link text-light" to="/orders" v-if="role === Roles.ADMIN"
              >Παραγγελίες<i class="fa-solid fa-truck-fast ms-1"></i
            ></router-link>
          </li>
          <li class="nav-item">
            <router-link class="nav-link text-light" to="/products" v-if="role === Roles.ADMIN"
              >Προιόντα<i class="fa-solid fa-tooth ms-1"></i
            ></router-link>
          </li>
          <li class="nav-item">
            <router-link class="nav-link text-light" to="/my-orders" v-if="role === Roles.DENTIST"
              >Oι Παραγγελίες μου<i class="fa-solid fa-truck-fast ms-1"></i
            ></router-link>
          </li>
          <li class="nav-item">
            <router-link class="nav-link text-light" to="/my-profile"
        ...
        <button type="button" class="btn btn-danger" @click="logout" :disabled="loading">
          ...
          <span v-else>Αποσυνδεθήτε <i class="fa-solid fa-arrow-right-from-bracket"></i></span>
        </button>
      </div>
    </div>
  </nav>
